...
public class Winner {
    ...
    public static void main(String[] args){
        ...
        List<String> winnersOfToursGreaterThan3500km = 
        		tdfWinners
					.stream()
					.filter(w -> w.getLengthKm() >= 3500)
					.map(Winner::getName)
													.collect(toList());
    	...
    	List<String> winnerObjectsOfToursLessThan3500kmLimit2 =  
    			tdfWinners
					.stream()
					.filter(d -> d.getLengthKm() < 3500)
					.limit(2)
        ...
        List<String> distinctTDFWinners = 
        		tdfWinners
        			.stream()
        			.map(Winner::getName)
        			.distinct()
        ...
        List<Winner> skipEveryOtherTDFWinner = 
        		tdfWinners
        			.stream()
        			.skip(2)
        			.collect(toList());
        ...
        List<Integer> mapWinnerNameLengthToListAnotherWay = tdfWinners
        		.stream()
        		.map(Winner::getName)
        		.map(String::length)
        		.collect(toList());
        
        System.out.println("mapWinnerNameLengthToList " + mapWinnerNameLengthToListAnotherWay);
